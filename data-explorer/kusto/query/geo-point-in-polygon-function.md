...
description: In diesem Artikel wird geo_point_in_polygon() in Azure Data Explorer beschrieben.
...
ms.locfileid: "81663762"
---
# <a name="geo_point_in_polygon"></a>geo_point_in_polygon()

Berechnet, ob sich die georäumlichen Koordinaten in einem Polygon auf der Erde befinden.

**Syntax**

`geo_point_in_polygon(`*longitude*`, `*Längengrad-Breitenpolygon*`, `*polygon*`)`

**Argumente**

* *Längengrad*: Georäumliche Koordinate, Längengradwert in Grad. Gültiger Wert ist eine reelle Zahl und im Bereich [-180, +180].
* *Breitengrad*: Georäumliche Koordinate, Breitengradwert in Grad. Gültiger Wert ist eine reelle Zahl und im Bereich [-90, +90].
* *Polygon*: Polygon im [GeoJSON-Format](https://tools.ietf.org/html/rfc7946) und eines [dynamischen](./scalar-data-types/dynamic.md) Datentyps.

**Rückgabe**

Gibt an, ob sich die räumlichen Koordinaten innerhalb eines Polygons befinden. Wenn die Koordinaten oder das Polygon ungültig sind, erzeugt die Abfrage ein Nullergebnis. 
...
> * Das [geodätische Datum,](https://en.wikipedia.org/wiki/Geodetic_datum) das für Messungen auf der Erde verwendet wird, ist eine Kugel. Polygonkanten sind Geodäsie auf der Kugel.

**Polygondefinition und Einschränkungen**

dynamic('type': "Polygon","koordinaten": [ LinearRingShell, LinearRingHole_1 ,..., LinearRingHole_N ])

* LinearRingShell ist erforderlich und `counterclockwise` definiert als geordnetes Koordinatenarray [[lng_1,lat_1],...,[lng_i,lat_i],...,[lng_j,lat_j],...,[lng_1,lat_1]]. Es kann nur eine Schale geben.
* LinearRingHole ist optional und `clockwise` definiert als geordnetes Koordinatenarray [[lng_1,lat_1],...,[lng_i,lat_i],...,[lng_j,lat_j],...,[lng_1,lat_1]]. Es kann eine beliebige Anzahl von Innenringen und Löchern geben.
* LinearRingsche müssen mit mindestens drei Koordinaten getrennt sein. Die erste Koordinate muss der letzten entsprechen; Daher sind mindestens vier Einträge erforderlich.
...
* LinearRingShell umschließt höchstens die Hälfte der Kugel. LinearRing teilt die Kugel in zwei Bereiche. Die kleinere der beiden Regionen wird ausgewählt.
* LinearRing Kantenlänge muss kleiner als 180 Grad sein. Die kürzeste Kante zwischen den beiden Scheitelpunkten wird gewählt.
* Lineare Ringe dürfen keine Kanten kreuzen und nicht gemeinsam nutzen. LinearRings kann Scheitelpunkte gemeinsam nutzen.
* Polygon enthält nicht unbedingt seine Scheitelpunkte. Punkteinschließung im Polygon ist so definiert, dass, wenn die Erde in Polygone unterteilt ist, jeder Punkt durch genau ein Polygon enthalten ist.

> [!TIP]
> Die Verwendung von Literalpolygonen kann zu einer besseren Leistung führen.

**Beispiele**

Manhattan-Insel ohne Central Park

:::image type="content" source="images/queries/geo/polygon_manhattan_with_hole.png" alt-text="Manhattan mit einem Loch":::
...
Sturmereignisse in Kalifornien. Die Ereignisse werden nach dem california State Polygon gefiltert und nach Ereignistyp und Hash aggregiert.
:::image type="content" source="images/queries/geo/california_storm_events.png" alt-text="Sturmereignis in Kalifornien":::
...
Das folgende Beispiel zeigt, wie Koordinaten in Polygone mit [Partitionsoperator](./partitionoperator.md) klassifiziert werden.
...
Im folgenden Beispiel wird ein Nullergebnis aufgrund der ungültigen Polygoneingabe zurückgegeben.
